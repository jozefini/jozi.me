...
import authorType from './lib/schemas/author'
import postType from './lib/schemas/post'
import settingsType from './lib/schemas/settings'

export default createConfig({
  basePath: '/studio',
  projectId: process.env.NEXT_PUBLIC_SANITY_PROJECT_ID,
  dataset: process.env.NEXT_PUBLIC_SANITY_DATASET,
  title: process.env.NEXT_PUBLIC_SANITY_PROJECT_TITLE || 'Sanity Editor',
  schema: {
  ...
  plugins: [
    ...
    unsplashImageAsset(), // Add an image asset source for Unsplash
  ...
  document: {
    ...
    newDocumentOptions: (prev, { creationContext }) => {
      if (creationContext.type === 'global') {
        return prev.filter((templateItem) => templateItem.templateId !== settingsType.name)
      }
      return prev
    },
